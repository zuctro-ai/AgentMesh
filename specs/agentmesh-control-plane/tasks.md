...
- [ ] 4. Extend Task Orchestrator (`core/orchestrator.py`)
  - [ ] 4.1 Add `pause_task_for_hitl(task_id, tool_name) -> AgentTask`
    - Sets status to `WAITING_HITL`, records `hitl_trigger_tool`
    - _Requirements: 3.1_
  - [ ] 4.2 Add `resume_task_from_hitl(task_id, decision, operator_id, reason) -> AgentTask`
    - APPROVED → QUEUED and re-enqueue; REJECTED → FAILED with reason
    - _Requirements: 3.3, 3.4_
  - [ ] 4.3 Write property test for HITL state machine round-trip
    - **Property 7: HITL Pause Prevents Worker Dequeue**
    - **Property 8: HITL Round-Trip (Approve Restores Queueable State)**
    - **Validates: Requirements 3.2, 3.3**
  - [ ] 4.4 Add per-task SSE event queue (`event_bus: Dict[str, asyncio.Queue]`)
    - Emit `TaskEvent` on every status transition
    - _Requirements: 8.1, 8.4_
  - [ ] 4.5 Add background coroutine `expire_stale_hitl_tasks(timeout_seconds)`
    - Scans WAITING_HITL tasks; transitions timed-out tasks to FAILED
    - _Requirements: 3.5_
  - [ ] 4.6 Add background coroutine `expire_stale_workers(timeout_seconds)`
    - Scans workers by last_heartbeat; transitions stale workers to OFFLINE
    - _Requirements: 6.4_
  - [ ] 4.7 Write property test for priority queue ordering
    - **Property 5: Priority Queue Ordering**
    - **Validates: Requirements 2.1**
  - [ ] 4.8 Write property test for retry-to-DLQ lifecycle
    - **Property 6: Retry Count Monotonically Increases to DLQ**
    - **Validates: Requirements 2.3, 2.4**

- [ ] 5. Extend Database Store (`core/database.py`)
  ...
  - [ ] 5.1 Write property test for chargeback attribution consistency
    - **Property 9: Chargeback Attribution Consistency**
    - **Validates: Requirements 5.2**

- [ ] 6. Implement MCP Proxy Gateway (`core/mcp_gateway.py`)
...
- [ ] 7. Implement Telemetry Emitter (`core/telemetry.py`)
...
- [ ] 10. Implement gRPC Service (`grpc/server.py`)
  ...
  - _Requirements: 9.2_
  - [ ] 10.1 Write property test for gRPC TaskEnvelope round-trip
    - **Property 12: gRPC TaskEnvelope Round-Trip**
    - **Validates: Requirements 9.3**

- [ ] 11. Wire worker lifecycle updates into polling and result submission paths
  ...
  - [ ] 11.1 Write property test for worker state round-trip
